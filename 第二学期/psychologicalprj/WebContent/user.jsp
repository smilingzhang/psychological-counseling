        ...
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <div class="modal-footer">
                    ...
                    <a id="cancel-btn" href="cancelConsultation?id=7" class="btn btn-primary">我要取消预约</a>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="contains user-contain">
        ...
        <div class="panel user">
            <div class="panel-body">
                <!--头像 方形-->
                <img src="${ctx }/images/${user.userHeadPath }" alt="${user.userNickName }_头像"  id="img-user-center" style="max-width:100px;max-height:100px;"/>
                <div class="intr">
                    <!--用户昵称-->
	                ...
	                <div class="panel-body">
	                    ...
	                    <div class="dir-nav">
	                        <ul class="nav nav-pills">
	                            <li <c:if test='${empty(consultState) || consultState=="未咨询" }'> class="active"</c:if>><a href="consultationRecord?consultState=未咨询">未开始</a></li>
	                            <li <c:if test='${!empty(consultState) and consultState=="已咨询" }'> class="active"</c:if>><a href="consultationRecord?consultState=已咨询">已完成</a></li>
	                            <li <c:if test='${!empty(consultState) and consultState=="已取消" }'> class="active"</c:if>><a href="consultationRecord?consultState=已取消">已取消</a></li>
	                        </ul>
	                    </div>
	                    <!--咨询列表-->
	                    <div class="directory-contain-list">
	                        ...
	                        <table style="table-layout:fixed">
		                        <c:if test="${!empty(crList) && crList.size()>0 }">
		                            ...
		                            <c:forEach items="${crList }" var="consulter">
			                            <tr>
			                                <td>
				                                <!--咨询师照片-->
			                                	<img style="float:left;margin-right:20px;" src="${ctx }/images/${consulter.getTeacher().getUser().getUserHeadPath() }" alt="${consulter.getTeacher().getUser().getUserNickName() }">
			                                	<div>
				                                    <!--咨询师名字-->
				                                    ...
				                                    <!-- 咨询师性别/年龄 -->
				                                    <span class="tag">${consulter.getTeacher().getUser().getUserSex() }/${consulter.getTeacher().getAge() }岁</span><br/>
				                                    <!-- 咨询师资质 -->
			                                ...
			                                <td>
			                                    ...
			                                    <c:if test="${consulter.getConsultationrecordMethod()=='vedio' }">
				                                    <span>咨询方式：线上音视频咨询</span><br/>
			                                    </c:if>
			                                    <c:if test="${consulter.getConsultationrecordMethod()=='audio' }">
				                                    <span>咨询方式：线上语音咨询</span><br/>
			                                    </c:if>
			                                </td>
			                                <c:if test="${consultState=='未咨询'}">
				                                ...
				                                <c:set var="targetDate" value="${dateutil:addDate(dateutil:getDate(),10*60*1000) }" scope="request"></c:set>
				                                <c:set var="startDate" value="${consulter.getConsultationrecordOrderTime() } ${consulter.getConsultationrecordStartTime() }" scope="request"></c:set>
				                                <c:set var="endDate" value="${consulter.getConsultationrecordOrderTime() } ${consulter.getConsultationrecordEndTime() }" scope="request"></c:set>
				                                <!-- 非面对面咨询且没到规定时间 -->
				                                <c:if test="${consulter.getConsultationrecordMethod()!='faceToFace' 
				                                				&& dateutil:compare(requestScope.targetDate,requestScope.startDate)==2 }">
				                                ...
				                                <!-- 若是线上咨询，且已经结束 -->
				                                <c:if test="${consulter.getConsultationrecordMethod()!='faceToFace'
				                                				&& dateutil:compare(dateutil:getDate(),requestScope.endDate)==1 }">
					                                <td><span class="disabled">进入咨询室</span></td>
				                                </c:if>
			                                </c:if>
			                                <c:if test="${consultState!='未咨询'}"><td></td><td></td></c:if>
			                            </tr>                            
		                            </c:forEach>
		                        </c:if>
		                        <c:if test="${consultState=='未咨询' && empty(crList) }">
		                        ...
		                        <c:if test="${consultState=='已咨询' && empty(crList) }">
	                        		<tr><td class="tag">您暂无已完成的咨询</td></tr>
		                        </c:if>
		                        <c:if test="${consultState=='已取消' && empty(crList) }">
	                ...
	                <div class="panel-body">
	                    ...
	                    <c:if test="${!empty(courseList) }">
	                        ...
	                        <div class="course-block">
	                            <!--课程图片-->
	                            <a href="${ctx }/course-intr?courseId=<jm:encrypt>${course.courseId}</jm:encrypt>"><img src="images/course.jpg" alt="${course.get('courseName') }"></a>
	                            <!--课程名-->
	                            <a class="title" href="${ctx }/course-intr?courseId=<jm:encrypt>${course.courseId}</jm:encrypt>">${course.get('courseName') }</a><br/>
	                            <!--教师名 -->
	                            <a href="${ctx }/consultdetail/showdetail?teacherId=${course.get('teacherId')}" class="tag">${course.get('userRealName') }</a><br/>
	                            <!--学习进度-->
	                            <!-- <span class="progress">已学习1/12</span><br/> -->
	                            <!--进入学习按钮-->
	                            <a class="btn btn-primary" href="${ctx }/joinstudy?&courseId=<jm:encrypt>${course.courseId }</jm:encrypt>&ifbc=<jm:encrypt>${ifbc}</jm:encrypt>">进入学习</a>
	                        </div>                    		
	                    	</c:forEach>
	                    </c:if>
		                        ...
		                        <c:if test="${!empty(listenList) && listenList.size()>0 }">
		                            ...
		                            <c:set var="endTime" value="${listen.get('listenrecordOrderTime')} ${listen.get('listenrecordEndTime') }" scope="request"></c:set>
			                            <tr>
	                            			<!--倾听者头像-->
			                            	<td>
			                            		<a href="${ctx }/consultdetail/showdetail?teacherId=${listen.get('teacherId') }"><img style="margin-right:30px;" src="${ctx }/images/${listen.get('userHeadPath') }" alt="${listen.get('userRealName') }"></a>
			                                    <!--倾听者名字-->
                ...
                <div class="panel-body">
                    ...
                    <div class="directory-contain-list">
                        ...
                        <div id="setting-1">
                            ...
                            <form action="" method="POST">
                                ...
                                <table class="setting-table">
                                    <tr>
                                    	<td class="tag" class="20%">姓名</td>
                                    	<td id="userRealName">
	                                      <c:if test="${empty(user.userRealName) }">未实名</c:if>
	                                      <c:if test="${!empty(user.userRealName) }">${user.getUserRealName() }</c:if>
	                                    </td>
	                                </tr>
                                    <tr><td class="tag">证件号</td><td id="userIdNumber">
                                      <c:if test="${empty(user.userIdNumber) }">未实名</c:if>
                                      <c:if test="${!empty(user.userIdNumber) }">${user.userIdNumber.substring(0,6)}*********${user.userIdNumber.substring(14,18) }</c:if>
                                    </td></tr>
                        ...
                        <div id="setting-2" style="display:none;">
							...
							<div  style="float: left">
	                        	<img src="${ctx }/images/${user.userHeadPath }" alt="${user.userNickName }_头像"  id="img-user-center-second" style="max-width:100px;max-height:100px;"/>
                        	</div>
                        </div>
                        ...
                        <div id="setting-3" style="display:none">
                            ...
                            <form action="" method="POST">
                                ...
                                <c:if test="${!empty(user.userPhone) and fn:length(user.getUserPhone())>0}">
                        ...
                        <div id="setting-4" style="display:none">
                            <form action="" method="POST">
                                <table class="setting-table">
                                	<c:if test="${!empty(user.userPhone) and fn:length(user.getUserPhone())>0}"><tr><td>原绑定手机号码</td><td id="show-old-phone">${user.getUserPhone().substring(0,3)}****${user.getUserPhone().substring(7,11)}</td></tr></c:if>
                                    <tr>
                                    	<td style="width:20%">新绑定手机号码</td>
                                    	<td  style="width:50%"><input maxlength="11" id="phoneNum" name="phoneNum" type="text" class="form-control" placeholder="11位手机号" style="width: 150px;display: block; width: 300px;float: left;" onkeyup="loginVerifyPhone4Alert()"></td>
                                     	<!--★★★ 错误信息，新绑定手机号发是否合法 -->
                                   		<td style="width:30%"><font name="loginErrMsg4Phone" id="loginErrMsg4Phone" class="">&nbsp;</font></td>
                                   	</tr>
                                    <tr>
                                    	<td>验证码</td>
                                        <td>
