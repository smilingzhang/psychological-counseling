...
public class ConsultOrderTeacherDao extends BaseDao<Teacher> {
	...
	/**
	 * 
	 * @desc:根据咨询师的id查询咨询师的头像
	 * @param teacherId
	 * @return:string
	 * @trhows
	 */
	public String selectTeacherImg(int teacherId) {
		Session session=sessionFactory.getCurrentSession();
		Query query=session.createQuery("select userHeadPath from  User u where u.userId=?");
		query.setParameter(0, teacherId);
		String img=(String) query.uniqueResult();
		return img;
	}
}
