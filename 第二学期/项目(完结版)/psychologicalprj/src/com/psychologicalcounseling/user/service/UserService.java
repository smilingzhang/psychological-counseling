...
public class UserService {
	...
	public void splitConsultList(User user) {
		...
		if(consultationRecords!=null && consultationRecords.size()!=0) {
			//2. 将元素按照预约时间排序
			...
			for(ConsultationRecord record : list) {
				System.out.println("咨询状态"+record.getConsultState());
	...
	public List<Map<String, Object>> queryCourseWithPaging(int uid, int pageNum) throws Exception {
		String sql = "select distinct course.courseId,courseName,teacherId,userRealName from user,course,courserecord where teacherId=user.userId and course.courseId=courserecord.courseId and courserecord.userId=?";
	...
	public List<Map<String, Object>> queryFavoriteCourse(int uid) throws Exception {
		String sql = "select distinct course.courseId,courseName,teacherId,userRealName" + 
