...
<html lang="zh-cn">
  ...
  <body>
    ...
    <%
     		...
     		Object countlist = request.getAttribute("countlist");
     		String dataPoint = gsonobj.toJson(countlist);
    ...
    <script type="text/javascript">
		window.onload = function() { 
		    ...
		    toUrl = localhostPaht + projectName +  "/" + controllerName;
		    console.log(toUrl);
		    $.ajax({
		        url:toUrl,
		        async:false,
    ...
    <div id="loading-shade">
        <div>
            <p><i class="icon icon-spin icon-spinner"></i><br/>加载中</p>
        </div>
    </div>
                ...
                <div class="board">
                    ...
                    <div id = "historical-record" class="panel">
                        <div class="panel-body">
                        	<h3>咨询记录分析</h3>
                           <form action="${ctx }/showchart" method = "post">
				     			<select name="time">   
				     			
							        <option value="a" <c:if test="${time == 'a'}">selected="selected"</c:if> >一周内</option>   
							        <option value="b"<c:if test="${time == 'b'}">selected="selected"</c:if>>一个月内</option>   
							        <option value="c"<c:if test="${time == 'c'}">selected="selected"</c:if>>三个月内</option>   
							        <option value="d"<c:if test="${time == 'd'}">selected="selected"</c:if>>半年内</option>   
							        <option value="e"<c:if test="${time == 'e'}">selected="selected"</c:if>>一年内</option>     

				     		    </select>
	     		    		<input type="submit" value="确认">
     		   			 </form> 

		            	<p> 共咨询了 ${ num}位用户  咨询时长共计：${totaltime }小时</p>
		            	
		            	
		            
							<div id="chartContainer" style="height: 180px; width: 80%;"></div>
 				...
 				<div class="board">
                    ...
                    <div id = "daily-visit" class="panel" frameborder="0" scrolling="no">
                        <div class="panel-body">
                           	<div id="main" class="daily-visit" style="width:900px;height:300px;" ></div>
                           
                        </div>
                    </div>
                    <div id = "rank-list" class="panel" frameborder="0" scrolling="no" style="height:340px">
                        <div class="panel-body">
                            <h3>2019年访问量最高Top5</h3>
							<div id="conclusion_max" style="height:300px;width:250px;"></div>
                        </div>
                    </div>
                    <div id = "monthly-volume" class="panel" frameborder="0" style="height:340px">
                        <div class="panel-body" >
                        	<h3>“及时倾听”咨询量月统计图</h3>
                            <!-- 下拉框 -->
                            <select id="months" data-options="required:true,editable:false" style="border: 1px solid #ccc;position:absolute;left:510px" onchange="changemonth()">
								...
								<option value="06" selected="selected" >六月</option>
								<option value="07" selected="selected" >七月</option>
								<option value="08" selected="selected" >八月</option>
								<option value="09" selected="selected" >九月</option>
							...
							<!-- 【月份】日历图 -->
                            <div id="main_1" class="monthly-volume" style="width:580px;height:400px;"></div>
                        </div>
                    </div>
                    	<script type="text/javascript">
						  //取到后台传来的json类型数据，转化成只有值的二维数组
							var s=${sessionScope.usercount};
							console.log(s);
							var json = s;
							var arr = [];
							for(let i in json) {
						    ...
						    var option_year = {
						        ...
						        //【每月】的日历图
						        var myChart_month = echarts.init(document.getElementById('main_1'));
						        var month='01';
						        var option_month;
						        option_month = {
						        visualMap: {
						    ...
						    //下拉框选择月份，改变【每月】的日历状态图
						    function changemonth(){
						    	month=$("#months").val();
						    	option_month.calendar.range="2019-"+month;
						        myChart_month.setOption(option_month);
						        return true;
						    }
						    ...
						    //************************************************************
						    //下面是计算一年当中访问量最多和最少的日期，并显示。
						        
						
						   //排序
						   function bubble(arr){
							   var i,len=arr.length,j,item;
							   for(i=0;i<len;i++){
								   for(j=0;j<len-1-i;j++){
						   ...
						   var bubble_arr=bubble(arr);
						   //计算最大（最小）五天的访问量  
						   var max="";
						   var min="";
						   var length=bubble_arr.length;
						   for(var i=0;i<5;i++){
							   
						   		max=max+'日期:'+bubble_arr[length-1-i][0]+"  "+'访问量'+bubble_arr[length-1-i][1]+"\n";
						   }
						   //document.getElementById('conclusion_max').innerText = max;
							
						   var max_date_data = [];
						   var max_visit_data = [];
						   for(var i=0;i<5;i++){
							   max_date_data[i] = bubble_arr[length-1-i][0];
							   max_visit_data[i] = bubble_arr[length-1-i][1];
						   }
						   console.log(max_date_data);
						   console.log(max_visit_data);						   
						   var conclusion_max_chart = echarts.init(document.getElementById('conclusion_max'));
						   var conclusion_max_option = {
								   xAxis: {
								        type: 'value'
								    },
								    yAxis: {
						  	...
						  	console.log(conclusion_max_option)
						   conclusion_max_chart.setOption(conclusion_max_option);
				    </script>
                </div>
				<div class="board">
                    <h1>业务预测</h1>
