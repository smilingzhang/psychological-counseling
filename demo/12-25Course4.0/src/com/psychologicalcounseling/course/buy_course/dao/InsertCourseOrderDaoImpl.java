package com.psychologicalcounseling.course.buy_course.dao;
...
public class InsertCourseOrderDaoImpl extends BaseDao<CourseOrder> {
	...
	public int insertCourseOrder(Date date,float price,int userId,int courseId,String orderId) {
		 int i =insert("insert into courseorder(courseId,userId,courseorderBuyTime,courseorderPrice,courseorderRandomId) values(?,?,?,?,?)", courseId,userId,date,price,orderId);
		 return i;
	}
}
