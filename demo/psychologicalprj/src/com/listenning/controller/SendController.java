...
public class SendController extends HttpServlet {
	...
	public void deal(HttpServletRequest request, HttpServletResponse response, HttpSession session) {
		...
		Map<String, List<String[]>> audioAddress2messages = (Map<String, List<String[]>>) application
				.getAttribute("audioAddress2messages");
		...
		if (roomType.equals("consult")) {
			videoAddress2messages = sendServiceImpl.videoSend(videoAddress2messages, videoChatAddress, myName,
					otherName, message);
			application.setAttribute("videoAddress2messages", videoAddress2messages);
		} else {
			audioAddress2messages = sendServiceImpl.audioSend(audioAddress2messages, audioChatAddress, myName,
					otherName, message);
