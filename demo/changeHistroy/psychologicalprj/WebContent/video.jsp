...
// 	    				videos[0].removeAttribute("controls");
						...
						// 咨询师 与 普通的用户所产生的挂断按钮是不同的
						var close = document.createElement('button');
	    				close.id = "close";
	    				close.innerHTML = "挂断";
	    				
	    				<c:if test="${user.userIdentity eq 1 }">
		    				close.onclick = function(){
		    					
		    			    	var ajax = new XMLHttpRequest();
								ajax.open("GET", "returnForm", true);
								ajax.send();
								...
								ajax.onreadystatechange = function() {
									if (ajax.readyState == 4 && ajax.status == 200) {
				    					...
				    					commentDiv.innerHTML = ajax.responseText;
				    					document.getElementById("videoWrapper1").appendChild(commentDiv);
									}
								};
		    				};
		    	    	</c:if>
...
// 	    				}
	    				...
	    				<c:if test="${user.userIdentity ne 1 }">	
