...
public class ConsultOrderDao extends BaseDao<ConsultationRecord> {
	...
	public boolean selectIsConsult(int teacherId, String orderTime, String startTime, String endTime) {
		...
		Query query = session.createQuery(
				"select c.teacher.teacherId,c.consultationrecordOrderTime,c.consultationrecordStartTime,c.consultationrecordEndTime,c.consultationrecordState from ConsultationRecord c");
		List<Object[]> list = query.list();
		for (int i = 0; i < list.size(); i++) {
			Object[] con = list.get(i);
			if ((Integer) con[0] == teacherId && con[1].equals(orderTime) && con[2].equals(startTime)
					&& con[3].equals(endTime) && "已支付".equals(con[4])) {
				return false;
			}
		}
		return true;
	...
	public void updateConsultState(int consultOrderId) {
		...
		Query query = session.createSQLQuery(
				"update consultationRecord set consultationrecordState=?,consultState=? where consultationrecordId=?");
